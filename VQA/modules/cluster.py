...
class TCAttention(nn.Module):
    ...
    def forward(self, q_dict, kv_dict, t_feat):
        q = q_dict['x']
        kv = kv_dict['x']
        B, Nq, C = q.shape
        Nkv = kv.shape[1]
        ...
        k = t_feat.unsqueeze(1).expand(-1, Nkv, -1)
        k = self.k(k).reshape(B, Nkv, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
...
class TCBlock(nn.Module):
    ...
    def forward(self, inputs, t_feat):
        ...
        t_feat = self.norm1(t_feat)

        # attn
        x = x + self.drop_path(self.attn(q_dict, kv_dict, t_feat))
...
if __name__ == '__main__':
    ctm = CTM(sample_ratio=0.75, embed_dim=512, dim_out=512, k=3)
    ...
    block1 = TCBlock(dim=512, num_heads=8)
